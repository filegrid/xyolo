...
现在的 Web 控制台改成了类似 1Panel 的纯左右工作台布局：

- 左侧固定导航栏负责切换 **数据集 / 训练 / 模型 / 评估 / 部署**
- 左侧导航支持折叠，折叠状态会保存在浏览器中
- 右侧内容区只展示当前模块；训练模块的 **新建 / 列表 / 模板** 作为内容头部的二级切换
- 语言、主题、环境信息和外部工具入口统一放在右侧内容头部操作区
